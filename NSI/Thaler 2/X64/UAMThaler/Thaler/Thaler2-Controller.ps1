...
set-Content -Path "C:\Packages\Thaler\Install.cmd" -Value "`"C:\Packages\Windows\System32\WindowsPowerShell\v1.0\powershell.exe`" -file `"$PackageFolder\Thaler2-installer.ps1`" -ThalerENV $ThalerENV -PackageFolder `"$PackageFolder`" -ServerAppData `"$ServerAppData`"" | Out-null
