...
foreach($item in $path){
    ...
    foreach ($SingleOSD in $OSD1){
        ...
        $Shortcut = $singleosdcontent.SOFTPKG.name
        $Target= $singleosdcontent.SOFTPKG.IMPLEMENTATION.CODEBASE.FILENAME
        $Returnval += New-Object PsObject -Property @{ AppName = "$($item.name)"; ShortcutName = "$Shortcut"; Target ="$Target"}
    }   
}

$Returnval | Sort-Object -Property "ShortcutName","Target" | Export-Csv 'c:\uam\Shortcuts1.csv'
 #| Select-Object -Property AppName, EnvironmentVariable, EnvironmentalPath, FileType |Sort-Object filetype -Descending | Export-Csv -Delimiter ',' 'C:\uam\uam.csv'
